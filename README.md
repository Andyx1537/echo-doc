...
1. ~~前端资源根缺省失效~~ **已修**（2026-08-29，`echo-client` 仓 `855b464` 之前那个提交）：
   ...
   原缺省的危险不在于失效，而在于**工作区里那份遗留副本还在**——
   旧路径能读到东西，只是过期物料，不裂开、不报错，比直接失败难发现得多。
   **该副本已于 2026-08-30 删除**（与本仓那份逐字节比对一致，且本仓那份已推到远端），
   两张用户上传挪进了本仓 `Echo-assets/runtime/uploads/`（被 `.gitignore` 挡住，不入库）。
   现在工作区只剩一个资源根，读错路径会直接报错，不会静默拿到旧物料。
