...
flowchart LR
  ...
  GW --> B04
  B04 --> B05[B05 Plaza / 作品墙读取]
  B05 --> B06[B06 作品评论 / 收藏]
  UI --> B07[B07 行为证据 / 假设 / 适配]
  B07 -.->|影子假设未进排序或生成| B02
  B07 -.->|影子假设未进排序或生成| B05
  B01 --> PG[(PostgreSQL)]
  B02 --> PG
...
实线表示代码中已有调用/存储边界，不自动代表验收通过；虚线表示已定义方向或未连通能力。有 `WorkStore` 时 `/plaza` 下发公开作品；未装配时仍走回忆卡且可返回空页。
...
| B03 生成建窗 | 生成状态、候选、确认结果、唯一建窗 | 定妆可走万相图生图；候选图经 `GeneratedMediaPublisher` 落本方盘；编码未配则失败，不挂供应商链 | 漫画/视频未做；进程内 Executor，重启不续跑任务 |
| B04 作品审核 | Work 生命周期、审核复用、投稿名额、允许操作 | pending 占用、凭证复用、驳回同条重提、`t_work_uk_author_inflight`；自制上传与复用公开都落 `t_work_moderation`；运营通过/驳回/下架/恢复带 CAS；驳回或下架可申诉一次，推翻只回待审 | 内容运营台已合；完整管理后台（看板/TOTP）未做 |
| B05 Plaza | 可见性、排序分页、匿名批次、作者作品墙 | `/plaza` 发公开 Work；匿名批次与 reqId 快照落库 | 网格与全屏都不记 `n`（作品 id 写不进卡曝光表）；无 WorkStore 时仍发卡 |
...
`t_work.sourceCardId` 对未软删 Work 有局部唯一索引：一张来源卡同时最多一个未删除作品；自上传允许 NULL。`mediaKey/posterKey` 是资源索引的逻辑关系而非 SQL FK。Work 保存 title/body/topicIds、mediaType/width/height/durationMs、aiGenerated、originType、status、visibility、发布审核时间和软删信息。正文属于 Work；Card 保持私域来源身份。自制上传 `pending` 与凭证复用公开都落 `t_work_moderation`；旧 `t_moderation.cardId` 不能审核 Work。运营通过后 `status=public` 且 `reviewedAt` 只写一次；下架后再上架不刷新该时刻。
...
| BE-G02 | B03 定妆可出图并落本方盘；漫画/视频未做 | 明确独立媒体产物单元；当前不把定妆成功外推为漫画/视频完成 |
| BE-G03 | 旁白消费四类快照的码（`private-onboarding-v2`）；不进 JSON/自由文本/资源 id | 漫画/视频仍不消费这些快照出图 |
...
| BE-G06 | Work 投稿名额、重提、凭证、并发唯一约束、运营通过/驳回/下架/恢复、作者申诉一次与主管维持/推翻已合 | 内容运营台已合；看板/TOTP/客服/收入仍未做 |
