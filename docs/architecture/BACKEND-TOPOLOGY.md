...
| B04 作品审核 | Work 生命周期、审核复用、投稿名额、允许操作 | pending 占用、凭证复用、驳回同条重提、`t_work_uk_author_inflight`；自制上传与复用公开都落 `t_work_moderation`；运营通过/驳回/下架/恢复带 CAS；驳回或下架可申诉一次，推翻只回待审 | 完整后台页未做 |
...
| BE-G06 | Work 投稿名额、重提、凭证、并发唯一约束、运营通过/驳回/下架/恢复、作者申诉一次与主管维持/推翻已合 | 完整后台页仍未做 |
