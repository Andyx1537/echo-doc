...
实线是已有代码入口/调用；虚线是已定目标、尚未完整实现。当前 `/plaza` 下发公开 `Work`，点进同一条作品详情；个人墙走 `/users/{id}/works`。私人回忆卡继续保留私域职责。全屏单卡层未做。
...
| FE-07 | Work列表无详情点击处理，Plaza旧链并存，WorkStatus缺uploading/submitting、无提交能力字段 | 2026-09-16 已落地：作品墙点进同一条详情；广场发公开 Work；`submissionCapability` 已读。全屏层仍未做 |
