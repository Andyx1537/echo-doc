...
| FE-02 | 上传入口仅初始/选宠阶段；选宠后问卷/总结无追加素材入口，“视频稍后再补”未兑现 | 产品已允许额外素材，前后端需在独立补素材差额中明确当前选宠后上传的恢复步骤，不将已做一张图闭环冒称额度采集完成 |
| FE-03 | 首次上传允许video，但前文要求首张image，预览统一img且选择首个image | 前端/后端：首视频错误反馈和可回到照片上传须实测；避免视频object URL当图片显示 |
