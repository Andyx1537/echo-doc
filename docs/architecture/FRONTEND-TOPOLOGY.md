...
flowchart TD
  ...
  Me -->|无宠物| B05[B05 PlazaScreen：公开作品瀑布]
  ...
  B05 --> B06[B06 两层评论 / 收藏]
  B06 -->|需绑定时原地回流| B01
  B05 --> B07[B07 Phase 0 行为采集]
  B02 -.-> B07
```

实线是已有代码入口/调用；虚线是已定目标、尚未完整实现。当前 `/plaza` 下发公开 `Work`，点进全屏单卡，左滑进作者主页，主页墙读公开作品；从主页再进全屏不卸掉主页；想说的话进同一条详情。私人回忆卡继续保留私域职责。全屏层不记 `n`。
...
| B05 广场 | 公开作品瀑布，点进全屏单卡，左滑进作者主页 | GET `/plaza` → `Work`；主页墙 GET `/users/{id}/works`；`WorkImmersiveScreen`，想说的话进 `WorkDetailScreen`；主页进全屏时主页藏在下面 | 匿名两小时一批约 30 条；网格与全屏都不记 `n` |
...
| FE-03 | 选宠应收图片 | 2026-09-15 已落地：`accept=image/*`，前后端拒视频（`asset_video_not_accepted`），页内提示，无专用失败页 |
...
| FE-07 | Work列表无详情点击处理，Plaza旧链并存，WorkStatus缺uploading/submitting、无提交能力字段 | 2026-09-16 已落地：作品墙点进同一条详情；广场发公开 Work；`submissionCapability` 已读。广场点进全屏、左滑进作者主页、主页墙读作品、主页进全屏不卸挂已做，仍不记 `n` |
...
- `components/PublishScreen.tsx`、`components/WorksFeedScreen.tsx`、`components/WorkImmersiveScreen.tsx`、`api/http.ts`、`types.ts`：作品上传/列表/广场全屏与当前缺口。
