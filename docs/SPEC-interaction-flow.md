...
| **Q-3** | 🔄 **已确认要做，等排期**（v0.2） | 🔴 **「严格弹一层」这条裁定目前落不了地**（§11 #3/#4/#7） | ✅ **已确认要做**，🔴 **但排在另一份规格之后**：`Echo/docs/SPEC-feed-surfaces.md`（两层信息流的数据源与配比）正在写，🔴 **「每层各带自己的流」正是那份文档要定的事**。前端拿两份规格**一起改、只改一次**。<br>🔴 **依赖**：`SPEC-feed-surfaces.md` 的**每层流归属定义**。⚠️ **那是另一条线的文件，本规格不碰它** |
