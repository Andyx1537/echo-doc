...
执行状态先读 [当前进展与两端架构对照](CURRENT-DELIVERY-STATUS.md)：包含七个已确认任务、精确缺失、前后端对应拓扑及当前验收边界。下方旧技术快照不能替代最新执行状态。


## 2026-09-09 建档采集边界与匿名唤醒（G-38）

切号后本阶段做受控唤醒，匿名只能走到手机确认。选宠锁定即结束采集，建档内不再补传；后续补素材必须与当前主体一致。选宠只收图片。客户端只存钥匙。生成中断由服务端交代。合入等做完再提。
...
开发计划必须先拆成最小逻辑单元：一个用户结果、一个核心对象/状态机、一个主责系统、一组最小边界接口和一套可独立通过的验收。产品全景和跨系统拓扑只作组合索引，不直接形成一个开发任务；各单元分别完成后，再建立单独的组合联调与整链验收。
...
| 2026-09-04 | G-35：设备凭据与评论治理补充 | 设备凭据只登录未绑手机的匿名账号；作者可治理但不可编辑他人评论；作品详情默认热门一级 3 条、每条二级预览 2 条，匿名展开经手机号登录原路续接 |
...
| 2026-09-05 | G-37：基础契约 v4 收口 | 冻结 Onboarding 状态机、Phase 0 v1 字典、分层保存期限、90 天审核凭证、评论恢复/分页、短信初值和审核组合并发 |
| 2026-09-09 | G-38：采集边界与匿名唤醒 | 本阶段做唤醒入口；选宠锁定后建档内不补传；选宠只收图片；生成中断由服务端交代 |
...
- 私域游客可提交生成前资料；选宠/肖像只收图片，视频不识别。选宠锁定后建档内不再补传。正式生成前进入独立手机号登录流程。
- 切号后可从「我」的账号区唤醒仍保留的匿名会话；唤醒后仍停在绑定墙。客户端不得提交 `accountId`。
...
- 作品详情评论默认“热门一级 3 条 × 每条最多 2 条二级回复”；有剩余显示服务端返回的“展开其余 N 条回复”。匿名展开先手机号登录并原路续接，已绑定用户可展开完整评论。
...
- 评论默认页长 20、最大 50，服务端不透明游标；作者隐藏仅在作品治理视图可恢复，普通面不披露治理身份。
...
- 建档状态由服务端固定为可恢复链，匿名可以完成上传、主体选择和问卷，正式生成前绑定；失败写操作结果并回到可重试主状态。
- Phase 0 v1 的事件、反馈、弱假设和动作闭集已冻结且全部影子运行；数据期限与画像清除见 `G-37/FC4-FC5`。
