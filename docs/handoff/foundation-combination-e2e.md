...
- 评论/收藏已接 PG；隔离库 `55433` schema `2026091404` 上广场→详情→三加二→收藏换仓储实例仍同一条。验完已停库。身份主链沿用旧证据。
...
| 4. 真库组合：评论收藏落 PG | `done` | `echo@a7e8513` | `FoundationCombinationPgTest` 1/1；schema `2026091404` @ 55433 | 验完已停库 |
...
- 评论/收藏表早就在 schema 里，仓储此前只写内存。换一套 Store 实例仍能读到，才算接上库。
