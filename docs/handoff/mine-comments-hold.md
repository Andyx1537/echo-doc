...
| 资源 | 状态 |
|---|---|
| echo-client `frontend/mine-comments-hold` | 占用 |
| echo-doc `docs/mine-comments-hold` | 占用 |
| echo | 本块未改后端 |
...
| 块 | 状态 | 提交 | 验证 | 备注 |
|---|---|---|---|---|
| 1. 开账本 | `doing` | | | 作品墙 / 收藏进评论不卸 |
| 2. 前端：评论压在墙上 | `todo` | | 浏览器：我的作品点一条 → 详情时墙仍挂着 → 返回仍是墙 | 验收看 DOM，不出新对比图 |
