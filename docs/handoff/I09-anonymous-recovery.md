...
| 4. 回填 I09 | `done` | `echo-doc` 本提交 | mock 重跑 PASS | 切号→我→唤醒→同一账号同一份四题绑定墙；不能生成。真 PG 未接；不合 develop |

## 下一块入口

```text
仓库：echo / echo-client
动作：有隔离 PG 再接真接口；合入等做完再提，现在不要合 develop
禁止：占用 5180/18080；打 Tag；改两份锁住的测试夹具
```
