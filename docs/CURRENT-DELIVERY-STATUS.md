...
| I08 | 前后端功能分支合入develop及整版标签 | 已合 develop，未打 Tag。后端 `echo@42c205f`，前端 `echo-client@abcb002`。夹具红灯不在公共区做 |
...
- 后端：`develop` @ `42c205f`（建档上传拒视频；广场 reqId 快照、匿名批次、评论收藏接 PG、投稿并发唯一索引仍在）。
- 前端：`develop` @ `abcb002`（建档按钮跟 `allowedActions`；上传只收照片；游客收藏 1002 出绑定提示仍在）。
- schema：2026091406（广场 reqId 快照表）。9 月 8 日旧联调栈 5180/18080 已停；不要再当占用。短信固定9999。
- 本轮：建档能力门（FE-04 / ON2）。`OnboardingApiTest` 13/13；vitest 17；mock 5176 上传页拒视频、候选两钮可点。不是浏览器整版 PASS。漫画/视频、真短信、打 Tag 仍后置。
