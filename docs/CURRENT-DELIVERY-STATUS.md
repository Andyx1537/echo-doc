...
| 2 | phone-account-resolution | 固定9999开发增量QA PASS，真实新号绑定，HTTP已有号切换通过；切号不迁资料已由专项测试锁住 | I09 mock + 隔离 PG 真接口已过；合入等做完再提；阿里云后置 |
...
| I09 | 切号后受控唤醒旧匿名会话 | mock 整链 + 隔离 PG 真接口已过。`echo@6d2b1b3` / `echo-client@5c4efde`。Auth 21/21。未合 develop |
...
- 本轮 I09：隔离 PG `2026090702` 上 Auth 21/21（含唤醒口）。前端 vitest 167/167，mock 浏览器重跑 PASS。未起常驻 18081，不合 develop。
