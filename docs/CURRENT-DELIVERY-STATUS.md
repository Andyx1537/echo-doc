...
- 后端：`develop` @ `1e8b27a`（旁白快照消费；功能头 `c2f3bca`）。
- 前端：`develop` @ `f45eabd`（回忆卡超时展示；功能头 `2fb9fa2`）。
- schema：2026091409（工单含 `appealing`；`appealAt` 只写一次）。9 月 8 日旧联调栈 5180/18080 已停；不要再当占用。短信固定9999。
- 本轮：回忆卡超时只展示服务端字段已合 `echo-client@f45eabd`。系统 Chrome 打开 `http://127.0.0.1:5191/?ops=cards`。漫画/视频、真短信、打 Tag、看板仍后置。
