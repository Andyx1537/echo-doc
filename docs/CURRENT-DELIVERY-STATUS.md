...
- 后端：`develop` @ `67d7fc1`（竖图定妆按图生图尺寸；候选旁白拒绝英文 JSON）。
- 前端：`develop` @ `f6bc539`（有真图不再叠 emoji）。
- schema：2026091403；开发API 18080、页面5180 他人占用不碰；真出图走独立 18081/5179/5433。短信固定9999。
- 本轮：上传肖像已在建档页走出三张万相绘本定妆。未开 `ECHO_IMGGEN_PROVIDER=wanx` 时仍是渐变色块。漫画/视频、真短信、打 Tag 仍后置。
