...
- 七单元可做块与私域可做恢复已合 develop。定妆图生图已接，须显式打开万相开关。漫画/视频、真短信仍后置；不得用百分比掩盖未过门项。
...
|---|---|---|---|
| 1 | private-onboarding-session | 已有实现；新号正常浏览器主链贯通。挂死收成、写后读、切号空草稿已合 develop。定妆图生图已接 | 漫画/视频、真短信后置；出图须开万相开关 |
...
| I05 | 真正宠物图片/漫画/视频生成；主体/素材/事实快照消费 | 定妆图生图已接万相（需 `ECHO_IMGGEN_PROVIDER=wanx` + 现有百炼钥匙）。漫画/视频仍未做 |
...
- 后端：`develop` @ `76b2d60`（定妆万相图生图）。
- 前端：`develop` @ `f6bc539`（有真图不再叠 emoji）。
- schema：2026091403；开发API 18080、页面5180 他人占用不碰；验收走 5177 mock。短信固定9999；视觉/生成使用开发适配器，非真实模型效果验收。
- 本轮：定妆真出图按上传肖像走万相 img2img。未开 `ECHO_IMGGEN_PROVIDER=wanx` 时仍是渐变色块。漫画/视频、真短信、打 Tag 仍后置。
