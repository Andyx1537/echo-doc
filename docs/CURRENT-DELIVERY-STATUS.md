...
| B05 Plaza | 公共瀑布流与全屏单卡 | 已合 develop：只发已公开作品，点进全屏单卡，想说的话进同一条详情；匿名批次与广场 reqId 快照已落库 |
...
| 4 | plaza-work-read | 广场只读公开作品；匿名两小时一批约 30 条，批次已落库。reqId 快照已落库（`echo@c9348a6`）。点网格进全屏单卡已合 `echo-client@c1bf7c9` | 账本 CLOSED；全屏不记 `n` |
...
- 前端：`develop` @ `c1bf7c9`（广场点进去进全屏单卡；功能头 `aff700a`）。
- schema：2026091409（工单含 `appealing`；`appealAt` 只写一次）。9 月 8 日旧联调栈 5180/18080 已停；不要再当占用。短信固定9999。
- 本轮：广场全屏单卡已合 develop（`echo-client@c1bf7c9`），不记 `n`。漫画/视频、真短信、打 Tag、完整后台页、左滑进主页仍后置。
