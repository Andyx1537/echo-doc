...
| I08 | 前后端功能分支合入develop及整版标签 | 已合 develop，未打 Tag。后端 `echo@b60ff77`，前端 `echo-client@28c6c3b`。夹具红灯不在公共区做 |
...
- 后端：`develop` @ `b60ff77`（选完候选下发 `set_consent`；建档上传拒视频、广场 reqId 快照、匿名批次、评论收藏接 PG、投稿并发仍在）。
- 前端：`develop` @ `28c6c3b`（授权页两钮跟 `set_consent`；建档按钮跟能力门；上传只收照片）。
- schema：2026091406（广场 reqId 快照表）。9 月 8 日旧联调栈 5180/18080 已停；不要再当占用。短信固定9999。
- 本轮：建档授权能力门。`OnboardingApiTest` 14/14；vitest 17；mock 5176 授权页两钮可点，同意后进确认。不是浏览器整版 PASS。漫画/视频、真短信、打 Tag、全屏层仍后置。
