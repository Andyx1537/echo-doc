...
- 每个用户同一时间只能有一个作品处于处理或审核链路，不允许并行提交 N 个作品。
- 单通道的目的是让用户只面对一个明确的进行中结果，不衍生独立进度窗口或任务中心。
- 用户统一从“我的作品墙”查看当前作品状态、审核原因和服务端允许的下一步动作；作品详情可承接单条操作，但不是第二套进度系统。
- 服务端必须原子判断用户是否还能提交，并返回占用中的 `workId/status` 和能力字段；前端不得通过扫描本地列表自行推断。
- 名额占用状态：`uploading/submitting/pending`。
- `uploading/submitting` 是作品正式受理前后的提交流程阶段；技术可将其实现为独立 `submissionStage`，但能力查询和用户展示必须与 `pending` 一起按占用处理，不能因未写入 `Work.status` 而漏掉并发保护。
...
- 作品评论的角色权限、两层回复、游客预览和删除树语义以 `SPEC-work-comments-and-favorites.md` 为唯一详细规格。
...
6. 用户级单通道是否由服务端并发保护，且所有状态与操作只在“我的作品墙”及单条详情呈现。
