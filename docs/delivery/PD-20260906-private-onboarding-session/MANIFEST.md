...
状态：`INTEGRATING`（身份依赖已解除，正常主链已通，异常与收尾仍待）
任务版本：`v1.8`

当前进展与精确缺失统一见 [执行清单](../../CURRENT-DELIVERY-STATUS.md)。2026-09-08已通过真实浏览器完成新号上传至确认建窗，恢复第四题亦通过；下方手机号404阻塞是历史记录，已被固定9999身份单元解除。当前不能标完整私域验收PASS。
...
|---|---|---|---|---|---|
| Onboarding 持久化与 API | 后端 | `echo` / `backend/private-onboarding-session` | Onboarding 表、领域、路由、迁移和专项测试 | 身份/生成边界契约桩 | 已固定候选 `be3d6a6` |
...
| 2026-09-06 | 真实 PostgreSQL 门禁 | 隔离 PostgreSQL 完整应用 schema `2026090601`；会话重建恢复、确认故障回滚、不同幂等键双请求竞争单窗均通过；BE `be3d6a6`，全专项 26/26 | `BLOCKED_DEPENDENCY` | `phone-account-resolution` 单元 |

### 7.1 固定候选与未过门证据

- 前端：`echo-client@d280246`，分支 `frontend/private-onboarding-session`。
- 后端：`echo@be3d6a6`，分支 `backend/private-onboarding-session`，schema 版本 `2026090601`。
- 后端专项：`OnboardingApiTest + PgOnboardingRepositoryTest + SchemaContractTest + UploadStorageTest + ResourceStoreTest`，共 26 项通过；其中 PostgreSQL 2 项在隔离真库执行，覆盖重启式仓储恢复、事务故障回滚和双请求竞争单窗。
...
- 唯一未通过组合依赖：前端需要的手机号 challenge/verify/resolution 由下一 `phone-account-resolution` 单元提供，当前请求 404，因此本候选无法完成真 UI 整链。该依赖交付前保持 `BLOCKED_DEPENDENCY`，不进入最终 `QA_VERIFYING`。
...
- 最终状态：本单元代码缺陷和 PostgreSQL 门禁已销账；待下一身份单元交付后进行真 UI 组合联调，未进入最终 QA/产品验收。
