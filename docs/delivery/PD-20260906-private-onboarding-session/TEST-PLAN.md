...
- 文档基线：`echo-doc/develop@5e00d2f`
- 基础冻结契约：`347e846`；称呼增量契约：`5e00d2f`，以 `API-CONTRACT §2.1` 为准
...
- 可选宠物称呼的默认、修改、恢复、幂等和 CAS，且独立于 Q1～Q4；
...
| ONB-025 | P0 | 分别以缺省称呼、自定义称呼和清空称呼创建/修改会话；刷新、登录返回、服务重启后恢复；同键重放、异载荷重放和并发旧版本修改 | 缺省/清空时快照规范化为“它”；自定义称呼由 snapshot 持久返回并在确认建窗后保持；不写入四题答案；同键同载荷返回原结果、异载荷为 `idempotency_conflict`、旧版本为 `onboarding_version_conflict`，竞争失败不覆盖成功值 |
...
前后端候选到达后，先完成 `QA-EXECUTION-CHECKLIST.md` 的 Gate A～C，再执行用例。任何契约字段、状态、权限、错误或幂等语义偏离基础契约 `347e846` 及称呼增量 `5e00d2f`，立即停止联调并把账本退回 `CONTRACT_REVIEW`，不得由 QA 自行选择兼容解释。
