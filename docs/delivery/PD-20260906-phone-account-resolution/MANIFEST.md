...
状态：`PRODUCT_ACCEPTING`（固定码开发联调增量 QA PASS；非生产归档）
任务版本：`v2.1`

### v2.1 当前差额与证据（2026-09-08，优先于下方 v2.0 历史记录）

- 用户确认：任意 11 位数字手机号 + 固定 `9999`；阿里云正式短信最后接，不阻塞本轮联调。
- 后端：显式 `-Decho.auth.mode=development-fixed-code`；挑战生成固定码，无真实短信发送，仍走原验证及绑定/切换事务；默认不启用。提交 `a2cb9d0`，启动 schema 常量补齐 `867d0a0`，均推送。实际 schema 为 `2026090702`，下方 `2026090701` 是历史记录。
- 前端：任意 11 位转 `+86`，支持 4/6 位输入，mock 固定 `9999`；发送幂等关联手机号，验证幂等关联 challengeId + code。提交 `b669edd`；165 项测试、构建通过。
- 三方设计回执：FE accepted-with-risks，BE accepted，QA accepted-with-risks；本次不改账号、数据迁移和长期凭据规则。
- 最终 QA：固定码开发联调差额 PASS，绑定 FE `b669edd` / BE `867d0a0`；QA 代码复核及后端报告，并采信主协调人前端测试/真实浏览器证据。仅覆盖本差额，不是整版发布 PASS。
- 后端验收：真实 PG/HTTP/装配专项 24/24，含全零号码 + 9999 新绑定/已有号切换及错误码拒绝；QA 后端差额 PASS。
- 真实前端→后端验收：本地前端 5180，API 18080，隔离 PostgreSQL；输入 22222222222、9999，确认绑定后返回原“我”页面并显示“已绑定账号”。本轮浏览器仅验证新绑定，已有账号切换有 HTTP 证据，尚无本轮浏览器证据。
- 替代：v2.0 将真实短信列为浏览器联调前置依赖的说法失效。剩余正式阿里云供应商适配/生产配置独立后置；不据此宣称整个产品版完成。
...
|---|---|---|---|---|---|
| 身份持久化、挑战/解析/会话 API | 后端 | `echo` / `backend/phone-account-resolution` | 身份表、凭据、会话、频控、SmsProvider 边界与专项测试 | 真实供应商待接 | 内部候选 `d8908a7`；PG/HTTP 21/21 + 运行时门禁 1/1 |
| 全局手机号登录组件与会话替换 | 前端 | `echo-client` / `frontend/phone-account-resolution` | 身份 API、会话/设备凭据存储、绑定/切号窗与 returnTo | 固定后端 DTO/错误 | 内部候选 `fdf35c5`；全量 165/165、构建与本地浏览器 PASS |
...
| 2026-09-07 | 后端/QA | 实际启动 JDK HTTP 网关并以网络客户端接真实 PG；设备会话、Bearer、绑定主链、幂等、旧入口 410 与 JSON 信封通过 | HTTP 第一批 `7e4aa3e`；联合 17/17 | `switch_existing` HTTP |
| 2026-09-07 | 后端/QA | `switch_existing` 实际 HTTP 链通过；目标账号签新会话、源会话失效、匿名恢复凭据隔离，源匿名账号与资料不迁移、不删除 | HTTP 切号 `f22c73b`；联合 18/18 | 断连重放 |
| 2026-09-07 | 后端/QA | 原始 Socket 发出完整 confirm 后在读取响应前断开；事务提交后同键取回同一结果，不同键返回已使用且无重复绑定/签发 | 断连重放 `312e8c1`；联合 19/19 | 重启与长期凭据 |
| 2026-09-07 | 后端/QA | 将账号、session/device credential 创建时间回填至 3 年前并重建身份服务/HTTP 网关；未显式撤销的长期凭据仍可鉴权和恢复 | 后端内部候选 `42c563a`；联合 20/20 | 真浏览器联调 |
| 2026-09-07 | 前端/产品 | 浏览器发现手机号弹窗无覆盖层及账号继承文案过宽；已补移动端弹窗、可访问标签并区分新号保留/旧号不迁移 | 前端 `c6579ff`；165/165、构建/浏览器 PASS | 验证码恢复 |
| 2026-09-07 | 后端/QA | 网关日志改用路由模板，confirm 的 resolutionToken 不再进入日志 | 后端 `1e68281`；PG/HTTP 21/21 | 运行时短信门禁 |
| 2026-09-07 | 前端/产品 | 消费 challenge 有效期与重发时间；展示倒计时，冷却后可重发，过期/锁定回到恢复态 | 前端 `fdf35c5`；165/165、构建/浏览器 PASS | 真短信联调 |
| 2026-09-07 | 后端/QA | 正式运行装配移除测试短信桩选择；即使遗留开发开关误配也只会明确失败，Stub 仅由测试直接注入 | 后端 `d8908a7`；门禁 1/1 | 真实供应商适配器 |

固定联调契约：`phone-account-resolution-v1 @ d6c2090`；前端 `fdf35c5`；后端 `d8908a7`；schema `2026090701`。
真实请求响应证据：真实 PostgreSQL + 服务层/实际 HTTP 网关联合专项 21/21，含断连、切号、服务重建与动态令牌脱敏；运行时短信桩门禁 1/1；前端全量 165/165、生产构建和本地 mock 浏览器通过。真实短信与真 FE↔BE 浏览器链待供应商接入后补，当前不得宣称生产端到端完成。

## 8. 缺陷闭环

| 缺陷 | 严重度 | 状态 | Owner | 定案/契约 | 修复提交 | 复验报告 |
|---|---|---|---|---|---|---|
| `nextAction=none` 被前端拒绝并保留已失效旧会话 | P0 | 已修复 | 前端 | `phone-account-resolution-v1` | `08e9067`，纳入 `cf11e19` | 前端全量 165/165、构建 PASS |
| confirm 敏感成功结果可永久重放 | P0 | 已修复 | 后端 | 短时交付窗，不改变长期凭据寿命 | `ecdc0a3` | 真实 PG 专项 14/14 |
...
| confirm 动态路径可能把 resolutionToken 写入日志 | P1 | 已修复 | 后端 | 日志只使用注册路由模板 | `1e68281` | PG/HTTP 联合 21/21 |
| 验证码无倒计时、重发和过期/锁定恢复 | P1 | 已修复 | 前端 | 5 分钟有效、60 秒冷却，服务端时间权威 | `fdf35c5` | 165/165、构建/浏览器 PASS |
| 运行时误配开发开关可能启用测试短信桩 | P1 | 已修复 | 后端 | Stub 只由测试直接注入 | `d8908a7` | 运行时门禁 1/1 |
...
- 最终状态：内部代码候选已收口；真实短信供应商适配、生产配置与真 FE↔BE 浏览器冒烟属于唯一剩余上线门禁，完成前不可归档为生产就绪。
