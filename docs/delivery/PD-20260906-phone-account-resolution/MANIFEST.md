...
|---|---|---|---|---|---|
| 身份持久化、挑战/解析/会话 API | 后端 | `echo` / `backend/phone-account-resolution` | 身份表、凭据、会话、频控、SmsProvider 边界与专项测试 | 无外部云账号时可先 stub | 后端内部候选 `42c563a`；联合专项 20/20 |
...
固定联调契约：`phone-account-resolution-v1 @ d6c2090`；前端 `cf11e19`；后端 `42c563a`；schema `2026090701`。
真实请求响应证据：真实 PostgreSQL + 服务层/实际 HTTP 网关联合专项 20/20，含断连和服务重建；前端全量 165/165 与生产构建通过。真浏览器证据待补，当前不得宣称端到端完成。
...
- 最终状态：后端内部候选已收口，绑定/切号、断连重放、历史长期凭据与服务重建均通过；真浏览器和真实短信冒烟尚未完成，不可归档。
