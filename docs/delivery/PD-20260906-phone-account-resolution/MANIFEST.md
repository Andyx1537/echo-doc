...
|---|---|---|---|---|---|
| 身份持久化、挑战/解析/会话 API | 后端 | `echo` / `backend/phone-account-resolution` | 身份表、凭据、会话、频控、SmsProvider 边界与专项测试 | 无外部云账号时可先 stub | HTTP 第一批候选 `7e4aa3e`；联合专项 17/17 |
...
固定联调契约：`phone-account-resolution-v1 @ d6c2090`；前端 `cf11e19`；后端 `7e4aa3e`；schema `2026090701`。
真实请求响应证据：真实 PostgreSQL + 服务层/实际 HTTP 网关联合专项 17/17；前端全量 165/165 与生产构建通过。`switch_existing` HTTP、断连重放、重启恢复与真浏览器证据待补，当前不得宣称端到端完成。
...
- 最终状态：双端安全收口候选已固定，绑定当前账号的真 HTTP 第一批通过；已有账号切换、断连/重启、真浏览器和真实短信冒烟尚未完成，不可归档。
