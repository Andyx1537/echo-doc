...
## 2026-09-08 手机号开发联调增量（PD-20260906-phone-account-resolution v2.1）

用户确认：本阶段任意 11 位数字手机号配固定验证码 `9999` 跑通流程；正式短信基本确定阿里云，最后接入，不阻塞本轮开发联调。前端把 11 位号码规范化为 `+86`，允许 4/6 位验证码提交，验证和账号变更仍由服务端决定。后端显式设置 `-Decho.auth.mode=development-fixed-code` 才生成 `9999`，不发送真实短信；默认模式不启用固定码。验证码挑战、绑定/切号、幂等、匿名资料保留及登录凭据不自然到期的既定规则不变。

此增量覆盖此前“运行时仅可失败、必须接真实短信才能继续浏览器联调”的描述；不表示阿里云已接入或生产已就绪。精确实现、三方回执和验收证据见 [手机号交付账本](delivery/PD-20260906-phone-account-resolution/MANIFEST.md)。
...
## 2. 建档 Onboarding（旧实现接口；由 §2.1 目标契约替代）

本节 `detect/start/refine/confirm` 仅登记当前实现和迁移来源，不再是新客户端契约。新客户端切换后停止调用；若发布盘点发现活跃旧端，最多兼容 30 天、最长 60 天，到期统一返回 `410 endpoint_retired`。
...
### 2.1 Onboarding v1 可恢复目标契约

所有修改请求必须携带 `Idempotency-Key` 和 `expectedSessionVersion`；服务端成功后递增 `sessionVersion`。重复幂等键同载荷重放原结果，不同载荷返回 `idempotency_conflict`；旧版本返回 `onboarding_version_conflict` 及最新快照。
...
- `PUT /works/:workId/comments/:commentId/hidden`：仅作品作者可隐藏；要求 `expectedStateVersion` 与 `Idempotency-Key`，返回 `{ commentId, displayState:"owner_hidden", stateVersion, visibleCommentCount }`。
- `DELETE /works/:workId/comments/:commentId/hidden`：仅作品作者治理视图可恢复；返回 `{ commentId, displayState:"visible", stateVersion, visibleCommentCount, sortRefreshRequired:true }`。平台终态处置不得被作者恢复。
- `GET /works/:workId/comment-governance?cursor=`：仅作品作者返回本人已隐藏项目、原因和恢复能力，不返回平台受限审计细节。
...
- Phase 0 v1 的字段闭集、逐事件唯一 emitter、允许 context 和动作参数 schema 以 `SPEC-behavior-evidence-and-adaptation.md §2.1.1/§2.2-2.4` 为唯一字典；未知名称返回 `dictionary_entry_unknown`，非法字段/类型返回 `context_field_not_allowed|context_field_invalid|action_parameter_not_allowed|action_parameter_invalid`。
