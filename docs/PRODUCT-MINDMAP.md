...
- **`G-37` 对 `S1′` 的范围修订**：匿名可在私域限额上传、选择主体和完成问卷；正式生成、公开互动、发布和导出仍需手机号验证绑定。匿名试填不是公开写权限，也不能产生 AI 成本。
- **`G-38`**：本阶段做切号后的受控唤醒，唤醒后仍停在绑定墙。选宠锁定即结束采集，建档内不补传；选宠只收图片。生成中断由服务端交代。
...
| **现结论** | `G-37/FC1`：匿名允许私域限额上传、主体选择和问卷；触发 AI、**记得**、**献花**、评论、发布公开和导出仍需可验证绑定 |
